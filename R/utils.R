...
# level = 0: checks if input is a graph
# level = 1: checks if the input is a list of graphs
# level = 2: checks if the input is a list of list of graphs, and so on
valid.input <- function(Graph,level = 0){
  if(level == 0) {
    return (methods::is(Graph,"igraph"))
  } else {
    if(methods::is(Graph,"list")) return (Reduce(f = "&",Map(f = function(x) { valid.input(x,level - 1) },Graph)))
    else return (FALSE)
  }
}
